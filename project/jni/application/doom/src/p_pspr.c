...
int P_SwitchWeapon(player_t *player)
{
  ...
  //lprintf(LO_INFO, "cur_w:%d, ", currentweapon);
  #if 0
  int j ;
  for(j = 0; j< NUMWEAPONS; j++) 
    player->powers[pw_strength] = true;
  #endif
#if 1
  do
    switch (*prefer++)
      ...
      case 1:
       ...
       #if 0
       if (player->ammo[am_clip])//edit by niuzb
          newweapon = wp_pistol;
          break;
       #endif
        if (!player->powers[pw_strength])      // allow chainsaw override        
  ...
  //lprintf(LO_INFO, "i:%d, ", i);
#else
  newweapon = (currentweapon+1)%NUMWEAPONS;
