...
static int I_TranslateKey(SDL_keysym* key)
{
  ...
  case SDLK_F6:   rc = /*KEYD_F6*/119;   break;//niuzb
  ...
  case SDLK_KP_PLUS:  rc = /*KEYD_KEYPADPLUS*/97; break;//niuzb
  case SDLK_KP_MINUS: rc = /*KEYD_KEYPADMINUS*/100;  break;//niuzb
...
int mouse_mode = 0;//0 means mouse up, 1 means down
...
static void I_GetEvent(SDL_Event *Event)
{
  ...
  case SDL_KEYDOWN:
    ...
    if(0){
     char a[50];
     sprintf(a, "down: %d, 0x%d\n", (Uint32)Event->type, (Uint32)Event->key.keysym.sym);
     logg(a);
    }
  ...
  case SDL_MOUSEBUTTONUP:
     ...
     {
     char a[50];
     sprintf(a, "UP: x:%d y:%d\n", Event->motion.x, Event->motion.y);
     logg(a);
    }
    return;//just return; by niuzb
  if (/*mouse_enabled*/0) // recognise clicks even if the pointer isn't grabbed
  {
    event.type = ev_mouse;
  ...
  if (/*mouse_currently_grabbed*/1) {//edit by niuzb
    event.type = ev_mouse;
  ...
  if(0){
     char a[50];
     sprintf(a, "SDL_MOUSEMOTION: x:%d y:%d\n", Event->motion.x, Event->motion.y);
     logg(a);
    }
    if(last_mouse_x == 0) {
      if(0){
       char a[50];
       sprintf(a, "last_mouse_x == 0:\n");
       logg(a);
      }
      last_mouse_x = Event->motion.x;
      last_mouse_y = Event->motion.y;
    ...
    else if((Event->motion.x-last_mouse_x) > 150 ||
            (Event->motion.x-last_mouse_x) < -150
           )
    {
      if(0){
       char a[50];
       sprintf(a, "(Event->motion.x-last_mouse_x) > 300\n");
       logg(a);
      }
    last_mouse_x = Event->motion.x;
    last_mouse_y = Event->motion.y;
    ...
    //if (mouse_mode == 0)///////////////xxxxxxxxxxxx
    //  return;///////////////////
    event.data2 = (Event->motion.x-last_mouse_x) << 5;
    event.data3 = /*-(Event->motion.y-last_mouse_y) << 5*/0;
    last_mouse_x = Event->motion.x;
    last_mouse_y = Event->motion.y;
    if(0){
     char a[50];
     sprintf(a, "rela: 0x%x, 0x%x x:%d y:%d\n", event.type, event.data1,event.data2>>5,event.data3>>5);
     logg(a);
    }
    #endif
    D_PostEvent(&event);
...
void I_UpdateVideoMode(void)
{
  ...
  if (V_GetMode() == VID_MODEGL) {
    ...
    //screen = SDL_SetVideoMode(SCREENWIDTH, SCREENHEIGHT, V_GetNumPixelBits(), init_flags);
      ...
      //sprintf(chars, "current video reo:%d, %d, ", myPointer->current_w,
      //  myPointer->current_h);
      //logg(chars);
    
    
    screen = SDL_SetVideoMode(myPointer->current_w, myPointer->current_h, 16, SDL_FULLSCREEN ) ; /* | SDL_HWSURFACE); */
  }

  if(screen == NULL) {
    logg("Couldn't set  video mode");
