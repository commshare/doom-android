...
class AccelerometerReader implements SensorEventListener {
	...
	public synchronized void onSensorChanged(SensorEvent event) {
		...
		if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) 
		{
			...
			//	nativeAccelerometer(Globals.TileSensitive*event.values[1], -event.values[0], event.values[2]);
			//else
		...
		{
			if( Globals.HorizontalOrientation )
				nativeOrientation(Globals.TileSensitive*event.values[1], -event.values[0], event.values[2]);
			else
